...
def isRotationMatrix(R):
    ...
    if R.ndim != 2 or R.shape[0] != R.shape[1]:
        return False
    isIdentity = np.allclose(R.dot(R.T), np.identity(R.shape[0], np.float64))
...
def estimateCameraPosition(focalLength, image_center, landmarks, vertices, rotAngles, translation):
    ...
    for i in range(focalLength.shape[0]):
        ...
        rots.append(rot)
        transs.append(trans)
    return torch.from_numpy(np.array(rots)).to(device=vertices.device, dtype=torch.float32), torch.from_numpy(np.array(transs)).to(device=vertices.device, dtype=torch.float32)
