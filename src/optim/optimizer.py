...
from geometry.projection import estimateCameraPosition
...
class Optimizer:
    ...
    def __init__(self, outputDir, config):
        ...
        os.makedirs(self.outputDir + '/checkpoints/', exist_ok=True)
    ...
    def saveOutput(self, samples,  outputDir = None, prefix = ''):
        ...
        try:
            ...
            if "out of memory" in str(e).lower() or "alloc" in str(e).lower():
                # Halve resolution and retry on OOM
                orig_w = self.pipeline.renderer.screenWidth
                orig_h = self.pipeline.renderer.screenHeight
                self.pipeline.renderer.screenWidth = orig_w // 2
                self.pipeline.renderer.screenHeight = orig_h // 2
                print(f"[WARN] OOM at {orig_w}x{orig_h}, retrying at "
                      f"{orig_w // 2}x{orig_h // 2}", file=sys.stderr, flush=True)
                images = self.pipeline.render(None, vDiffTextures, vSpecTextures, vRoughTextures)
                self.pipeline.renderer.screenWidth = orig_w
                self.pipeline.renderer.screenHeight = orig_h
            else:
                raise
