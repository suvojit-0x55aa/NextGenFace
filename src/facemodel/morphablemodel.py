...
from facemodel.meshnormals import MeshNormals
import numpy as np
import torch
import h5py
import sys
...
class MorphableModel:
    ...
    def __init__(self, path, textureResolution = 256, trimPca = False, landmarksPathName = 'landmark_62_mp.txt', device='cuda'):
        ...
        landmarks_data = np.genfromtxt(pathLandmarks, dtype=np.int64)
        self.landmarksAssociation = torch.tensor(landmarks_data[:, 1]).to(device)
        self.landmarksMask = torch.tensor(landmarks_data[:, 0]).to(device)
